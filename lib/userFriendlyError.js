...
const toUserMessage = (error, context = 'generic') => {
  ...
  if (context === 'download') {
    if (
      status === 404 ||
      /no stream url|stream resolver timed out|vidsrc\.sbs|hls playlist/i.test(message)
    ) {
  ...
  if (context === 'stream') {
    if (
      status === 404 ||
      /no stream url|stream resolver timed out|vidsrc\.sbs/i.test(message)
    ) {
      return "This title isn't available to stream right now. Please try again later.";
    }
    return "The stream couldn't start. Please try again.";
  }
